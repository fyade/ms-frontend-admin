...
<template>
        ...
        <el-table-column fixed="right" label="操作" min-width="200">
            <template #default="{ row }">
                ...
                <el-button :disabled="Object.keys(downloadProgresses).indexOf(row.id) > -1" variant="text"
                    ...
                    下载
                    <template v-if="Object.keys(downloadProgresses).indexOf(row.id) > -1">({{ downloadProgresses[row.id]
                        }}%)
                    </template>
                </el-button>
            </template>
    ...
    <br />
    <el-pagination ref="paginationref" v-model="pageParam.pageNum" v-model:pageSize="pageParam.pageSize"
        :total="state.total" show-jumper @change="onChange" />
</template>
