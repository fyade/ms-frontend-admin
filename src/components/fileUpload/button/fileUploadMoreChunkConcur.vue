...
<template>
  <el-button :loading="isLoading" :disabled="isDisabled" theme="primary" @click="upload7" :icon="Upload">
    ...
    <template v-if="isDisabled">&nbsp;
      ...
      <!--        (`${state.fileNum}/${state.fileTotal},` + `${state.progress_started}/${state.progress_ended}/${state.progress_total}`)-->
      <!--  }})</span>-->
      <span>(
        {{
          state.currentStage === 'a' ? state.dictStage[state.currentStage] :
              (`${state.fileNum}/${state.fileTotal},` + (['b', 'e'].indexOf(state.currentStage) > -1 ? state.dictStage[state.currentStage] : `${state.progress_started}/${state.progress_ended}/${state.progress_total}`))
        }}
        )</span>
    </template>
  </el-button>
